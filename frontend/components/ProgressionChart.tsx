...
import type { TwinSnapshot } from "@/types/twin";
      ...
      >
        ...
        {[0, Math.floor((n - 1) / 2), n - 1].map((i) => (
          <text
            ...
            fontSize={9}
          >
            {fmtDate(snapshots[i].last_updated_at)}
          </text>
        ))}
      </svg>
    </div>
...
function buildSeries(snaps: TwinSnapshot[]): Record<Metric, number[]> {
  // barrier_integrity and hydration_index are 0–1 in our type; scale to 0–100 for the chart.
  // activeFlareUps is a count; scale to 0–100 axis for co-display.
  const maxFlare = Math.max(1, ...snaps.map((s) => flareUpCount(s.active_flare_ups)));
  return {
    barrierIntegrity: snaps.map((s) => s.barrier_integrity * 100),
    hydrationIndex: snaps.map((s) => s.hydration_index * 100),
    activeFlareUps: snaps.map((s) => (flareUpCount(s.active_flare_ups) / maxFlare) * 100),
  };
}
